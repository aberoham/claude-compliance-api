...
```text
Compliance access keys  0
Compliance access keys are owned by the organization and remain active even after the creator is removed
[ + Create key ]

KEY                 SCOPES                  CREATED AT
--------------------------------------------------------------------------------
                    No compliance access keys found.
```
...
Name the key and select its scopes.

```text
Create compliance access key
Name your key:
[ e.g., Security Auditing Key                      ]

Scopes
[ ] read:compliance_activities (i)
[ ] read:compliance_user_data (i)
[ ] delete:compliance_user_data (i)

                              [ Create ] [ Cancel ]
```

Receive the key and store securely.

```text
Save your compliance access key
Keep a record of the key below. You won't be able to view it again.

sk-ant-api01-ABCDEFGHIJK_LMNOPQRSTUVWXYZabcd_e_fghijklmnopqrstuvwxyz012_3456789_ABCDEFGHIJKLMNOpQRS-TUVWXYZa
                                                                    [ Copy Key ]
...
#### Created keys appear in the list

```text
Compliance access keys  1
Compliance access keys are owned by the organization and remain active even after the creator is removed
[ + Create key ]

KEY                 SCOPES                                          CREATED AT
--------------------------------------------------------------------------------
Audit Key           read:compliance_activities                      Aug 12, 2025   [...]
sk-ant-api01-       read:compliance_user_data
KLj...RgAA          delete:compliance_user_data
```

#### Keys can be disabled or deleted

When a key is disabled it cannot be used to make requests to the API and can be later re-enabled. When a key is deleted it is permanently deleted and can no longer be used.

```text
KEY                 SCOPES                                          CREATED AT
--------------------------------------------------------------------------------
Audit Key           read:compliance_activities                      Aug 13, 2025   [...]
sk-ant-api01-       read:compliance_user_data                                      | (/) Disable access key |
hSv...NQAA          delete:compliance_user_data                                    | (🗑) Delete access key  |
```

#### A disabled key can be re-enabled

```text
KEY                 SCOPES                                          CREATED AT
--------------------------------------------------------------------------------
Audit Key (/)       read:compliance_activities                      Aug 13, 2025   [...]
sk-ant-api01-       read:compliance_user_data                                      | (/) Re-enable access key |
hSv...NQAA          delete:compliance_user_data                                    | (🗑) Delete access key    |
```
...
|---|---|---|---|
| `organization_ids[]`<br><br>*Repeatable* | `array` | Filter by organization IDs (accepts `org_...` or organization UUID) | `organization_ids[]=org_123abc&organization_ids[]=org_234bcd`<br><br>Repeat the `&organization_ids[]=...` parameter for every `organization_id` you want to include in the filter. |
| `actor_ids[]`<br><br>*Repeatable* | `array` | Filter by user | `actor_ids[]=user_abc123 &actor_ids[]=user_bcd234`<br><br>Repeat the `&actor_ids[]=...` parameter for every `actor_id` you want to include in the filter. |
| `activity_types[]`<br><br>*Repeatable* | `array` | Filter by activity types | `activity_types[]=claude_file_viewed&activity_types[]=api_key_created`<br><br>Repeat the `&activity_types[]=...` parameter for every `activity_type` you want to include in the filter. |
...
| `first_id` | `string` | Cursor for pagination. Set this value as the `before_id` query parameter to retrieve the previous page of results (heading forwards in time).<br><br>Clients should treat this value as an opaque string and not attempt to parse or interpret its contents, as the format may change without notice. |
| `last_id` | `string` | Cursor for pagination. Set this value as the `after_id` query parameter to retrieve the next page of results (heading backwards in time).<br><br>Clients should treat this value as an opaque string and not attempt to parse or interpret its contents, as the format may change without notice. |
...
Represents a change made by an Identity Provider (IdP) through SCIM directory sync. These activities are generated when your IdP pushes user and group changes to Claude through a directory sync connection (e.g., Okta, Azure AD, JumpCloud).
...
Represents an action performed via the Admin API using an Admin key. These activities are generated when an organization admin uses an Admin key to manage users, invites, workspaces, or API keys through Claude Console or Claude API.
  ...
  "data": [
    {
      ...
      // Additional fields specific to type:
...
| `scoped_api_key_deleted` | `api_key_id`, `api_key_name`, `scopes` | A scoped API key was permanently deleted (soft delete). |
| `scoped_api_key_updated` | `api_key_id`, `updates` | A scoped API key was renamed or its activation state changed. |

### Accounts

| Activity Type | Additional Fields | Description |
|---|---|---|
| `account_deleted` | (none) | User-initiated self-service account deletion. |
| `admin_request_created` | `request_type` | Admin request created by an org member (seat upgrade, limit increase, join org, end-user invite). |
| `domain_claim_initiated` | (none) | Domain capture claim initiated over personal accounts on verified domains. |
| `end_user_invite_requested` | `invitee_email` | Non-admin member submitted an invite request for a new org member. |
| `org_join_proposal_decided` | `approved` | Approve or reject decision on a parent-org join proposal. |
| `organization_icon_deleted` | (none) | Organization's custom icon deleted. |
| `organization_icon_updated` | (none) | Organization's custom icon uploaded or replaced. |
...
| `claude_artifact_published` | `claude_published_artifact_id`, `artifact_type`, `title` | An artifact was published and made publicly accessible. |
...
| `claude_published_artifact_deleted` | `claude_published_artifact_id` | A published artifact was unpublished/deleted by its creator. |
...
| Activity Type | Additional Fields | Description |
|---|---|---|
| `billing_emails_updated` | `primary_email_set`, `to_email_count`, `cc_email_count` | Billing email recipients were updated on the Stripe customer. |
...
| `invoice_collection_method_updated` | `new_collection_method` | Invoice collection method was changed. |
| `managed_organization_setup_completed` | (none) | Managed (AWS Marketplace) organization setup was completed. |
| `payment_method_updated` | (none) | Default payment method was updated on Stripe customer. |
| `platform_spend_limit_alert_emails_updated` | `alert_emails`, `alerted_roles` | Spend limit alert email addresses and role targets were updated for an org. |
| `platform_spend_limit_created` | `limit_usd`, `limit_action` | An org-level fixed-dollar spend limit was created. |
| `platform_spend_limit_deleted` | `spend_limit_id` | An org-level spend limit was removed. |
| `platform_spend_limit_updated` | `spend_limit_id`, `ignore` | An org-level spend limit snooze/ignore state was changed. |
| `prepaid_auto_recharge_disabled` | (none) | Auto-recharge was disabled for API prepaid org. |
| `prepaid_auto_recharge_updated` | `target_amount`, `threshold_amount` | Auto-recharge settings were updated for API prepaid org. |
...
| `seat_tier_changes_cancelled` | (none) | Scheduled seat tier downgrades were cancelled. |
| `seat_tiers_purchased` | `item_allocations` | Seat tiers were purchased or upgraded on a subscription. |
| `subscription_cancellation_scheduled` | (none) | Subscription cancellation was scheduled at end of billing period. |
| `subscription_renewed` | `plan_type`, `billing_interval` | A cancelled subscription was renewed. |
| `subscription_resumed` | (none) | A scheduled subscription cancellation was reversed. |
| `subscription_started` | `plan_type`, `billing_interval`, `seat_count` | A new subscription was created (team or ss-enterprise). |
| `subscription_upgraded` | `old_plan`, `new_plan` | Subscription plan was upgraded (e.g. Team to Enterprise). |
| `workspace_member_spend_limit_created` | `workspace_id`, `account_id`, `limit_usd`, `limit_action` | A per-member or workspace-default Claude Code spend limit was created. |
| `workspace_member_spend_limit_deleted` | `workspace_id`, `account_id`, `spend_limit_id` | A per-member or workspace-default Claude Code spend limit was deleted. |
| `workspace_member_spend_limit_updated` | `workspace_id`, `account_id`, `new_limit_usd`, `spend_limit_id` | A per-member Claude Code spend limit amount was updated. |
| `workspace_spend_limit_created` | `workspace_id`, `limit_usd`, `limit_action` | A workspace-level API spend limit was created. |
| `workspace_spend_limit_deleted` | `workspace_id`, `spend_limit_id` | A workspace-level API spend limit was deleted. |
...
| Activity Type | Additional Fields | Description |
|---|---|---|
| `claude_code_review_config_updated` | `enabled`, `environment_id`, `model`, `per_review_limit_usd` | Claude Code Review configuration was enabled/disabled for an org. |
| `claude_code_review_repository_added` | `config_id`, `repo_owner`, `repo_name`, `trigger_mode` | A repository was added to org-level Claude Code Review configuration. |
| `claude_code_review_repository_removed` | `config_id`, `repo_owner`, `repo_name` | A repository was removed from org-level Claude Code Review configuration. |
| `claude_code_review_repository_updated` | `config_id`, `repo_owner`, `repo_name`, `trigger_mode`, `status` | A Claude Code Review repository configuration was updated. |
| `claude_code_security_center_config_updated` | `enabled`, `environment_id` | Claude Code Security Center scanning was enabled/disabled for an org. |
...
| `claude_skill_disabled` | `skill_id`, `skill_name` | User disabled a skill for their account. |
| `claude_skill_enabled` | `skill_id`, `skill_name` | User enabled a skill for their account. |
...
| Activity Type | Additional Fields | Description |
|---|---|---|
| `audit_log_export_accessed` | (none) | Audit log export file was accessed/downloaded via signed URL. |
| `audit_log_export_started` | `from_date`, `to_date` | Audit log export was initiated. |
...
| `org_data_export_accessed` | (none) | Organization data export file was accessed/downloaded via signed URL. |
...
| `org_user_left` | `previous_role` | User removed themselves from organization. |
...
| `organization_address_updated` | `billing_name_updated`, `billing_address_updated`, `shipping_name_updated`, `shipping_address_updated` | Organization billing/shipping address was updated in Stripe. |
| `owned_projects_access_restored` | `user_id` | Access to owned projects was restored. |
| `primary_owner_transferred` | `previous_owner_id`, `new_owner_id` | Primary owner role was transferred to another org member. |
...
| `subscription_quantity_updated` | `previous_quantity`, `new_quantity`, `added_seats` | Contracted subscription seat quantity was updated. |
...
| `platform_federation_issuer_archived` | `federation_issuer_id` | An OIDC federation issuer was archived. |
| `platform_federation_rule_archived` | `federation_rule_id` | An OIDC federation rule was archived. |
| `platform_service_account_archived` | `service_account_id` | A service account was archived. |
...
| Activity Type | Additional Fields | Description |
|---|---|---|
| `cli_plugin_exec_policy_updated` | `plugin_id`, `marketplace_id`, `plugin_name`, `cli_name`, `op_name`, `max_permission` | Admin set or cleared the per-op permission ceiling for a plugin CLI. |
...
| Activity Type | Additional Fields | Description |
|---|---|---|
| `claude_pubsec_identity_configured` | `idp_saml_config_updated`, `magic_link_toggled`, `magic_link_enabled` | SAML IdP configuration updated for a public sector organization. |
...
| Activity Type | Additional Fields | Description |
|---|---|---|
| `platform_signing_key_created` | `signing_key_id`, `key_backing_type`, `algorithm`, `status` | Activity logged when a new request-signing key is registered for the org. |
| `platform_signing_key_deleted` | `signing_key_id`, `key_backing_type`, `algorithm`, `key_name` | Activity logged when a signing key is permanently deleted. |
| `platform_signing_key_rotated` | `old_signing_key_id`, `new_signing_key_id`, `key_group_identifier`, `algorithm` | Activity logged when an in-memory signing key is rotated. |
...
| Parameter | Type | Description |
|---|---|---|
| `organization_ids[]`<br>_Repeatable_ | `array` | Filter by organization IDs (accepts `org_...` or organization UUID) |
| `project_ids[]`<br>_Repeatable_ | `array` | Filter by project IDs (accepts `claude_proj_...` ) |
| `user_ids[]`<br>_Repeatable_ | `array` | Filter by user IDs |
...
Reference to an artifact version generated by the assistant.

Use `GET /v1/compliance/apps/artifacts/{version_id}/content` to download the full artifact content.
...
| `user` | `ComplianceProjectUser` or null | Project creator information, or null if the creator's account has been deleted |
...
Extends `ComplianceProject`, inheriting all its fields.
...
| `user` | `ComplianceProjectUser` or null | Document creator information, or null if the creator's account has been deleted |
...
curl -H "x-api-key: $ANTHROPIC_COMPLIANCE_ACCESS_KEY" \
  "https://api.anthropic.com/v1/compliance/apps/projects?user_ids[]=user_abc123"
...
```json
{
  "error": {
    ...
    "message": "The `created_at.gte` parameter contains an invalid timestamp format. Timestamps must be provided in RFC 3339 format."
  }
}
```
...
```json
{
  "error": {
    "type": "authentication_error",
    "message": "The API key provided does not have the `read:compliance_activities` scope required for this endpoint. Got: []"
  }
}
```
...
```json
{
  "error": {
    "type": "authentication_error",
    "message": "The API key provided does not have the `read:compliance_org_data` scope required for this endpoint. Got: []"
  }
}
```
...
```json
{
  "error": {
    "type": "authentication_error",
    "message": "The API key provided does not have the `read:compliance_user_data` scope required for this endpoint. Got: []"
  }
}
```
...
```json
{
  "error": {
    "type": "authentication_error",
    "message": "The API key provided does not have the `delete:compliance_user_data` scope required for this endpoint. Got: []"
  }
}
```
...
**FEATURE:** The following new activity categories have been added to the activity feed:

*   **Accounts:** `account_deleted`, `admin_request_created`, `domain_claim_initiated`, `end_user_invite_requested`, `org_join_proposal_decided`, `organization_icon_deleted`, `organization_icon_updated`
*   **Claude Code:** `claude_code_review_config_updated`, `claude_code_review_repository_added`, `claude_code_review_repository_removed`, `claude_code_review_repository_updated`, `claude_code_security_center_config_updated`
*   **Plugins:** `cli_plugin_exec_policy_updated`
*   **Pubsec:** `claude_pubsec_identity_configured`
*   **Signing Keys:** `platform_signing_key_created`, `platform_signing_key_deleted`, `platform_signing_key_rotated`

**FEATURE:** The following activity types have been added to existing categories:

*   **API Keys:** `scoped_api_key_deleted`, `scoped_api_key_updated`
*   **Artifacts:** `claude_artifact_published`, `claude_published_artifact_deleted`
*   **Billing:** `billing_emails_updated`, `invoice_collection_method_updated`, `managed_organization_setup_completed`, `payment_method_updated`, `platform_spend_limit_alert_emails_updated`, `platform_spend_limit_created`, `platform_spend_limit_deleted`, `platform_spend_limit_updated`, `prepaid_auto_recharge_disabled`, `prepaid_auto_recharge_updated`, `seat_tier_changes_cancelled`, `seat_tiers_purchased`, `subscription_cancellation_scheduled`, `subscription_renewed`, `subscription_resumed`, `subscription_started`, `subscription_upgraded`, `workspace_member_spend_limit_created`, `workspace_member_spend_limit_deleted`, `workspace_member_spend_limit_updated`, `workspace_spend_limit_created`, `workspace_spend_limit_deleted`
*   **Customizations:** `claude_skill_disabled`, `claude_skill_enabled`
*   **Organization Settings:** `audit_log_export_accessed`, `audit_log_export_started`, `org_data_export_accessed`, `org_user_left`, `organization_address_updated`, `primary_owner_transferred`, `subscription_quantity_updated`
*   **Platform Org Management:** `platform_federation_issuer_archived`, `platform_federation_rule_archived`, `platform_service_account_archived`

**FEATURE:** `ComplianceProjectResponse.user` and `ComplianceProjectDocument.user` may now be `null` when the project or document creator's account has been deleted.
...
*135 of 172 audit log events have a corresponding compliance activity.*
...
| `org_user_left` | `org_user_left` |
| - | `account_migration_started` |
...
| - | `connector_request_created` |
| - | `connector_request_approved` |
| - | `connector_request_dismissed` |
...
| `claude_gdrive_integration_created`,<br>`claude_github_integration_created` | `integration_org_enabled` |
| `claude_gdrive_integration_deleted`,<br>`claude_github_integration_deleted` | `integration_org_disabled` |
| `claude_gdrive_integration_updated`,<br>`claude_github_integration_updated` | `integration_org_config_updated` |
...
| - | `org_federation_issuer_created` |
| - | `org_federation_issuer_deleted` |
| - | `org_federation_issuer_archived` |
| - | `org_federation_rule_created` |
| - | `org_federation_rule_deleted` |
| - | `org_federation_rule_archived` |
| - | `org_service_account_created` |
| - | `org_service_account_archived` |
